...
export const FilterBar = ({allRoutes, setSelectedSetter, setSelectedGrade, setSelectedType, setSelectedStyle, setDisplayedRoutes}) => {
    ...
    return (
        <div className="filter-bar">
            <select onChange={(event) => {setSelectedType(event.target.value)}} id="type-select" className="route-search">
                <option value="0" id="type">select route type...</option>
                    {types.map(type => {
                        return <option value={type.id} key={type.id}>{type.name}</option>
                    })}
            </select>
            <select onChange={(event) => {setSelectedGrade(event.target.value)}} id="grade-select" className="route-search">
                <option value="0" id="grade">select grade...</option>
                    {grades.map(grade => {
                        return <option value={grade.id} key={grade.id}>{grade.name}</option>
                    })}
            </select>
            <select onChange={(event) => {setSelectedStyle(event.target.value)}} id="style-select" className="route-search">
                <option value="0" id="style">select a route style</option>
                    {styles.map(style => {
                        return <option value={style.id} key={style.id}>{style.name}</option>
                    })}
            </select>
            <select onChange={(event) => {setSelectedSetter(event.target.value)}} id="setter-select" className="route-search">
